...
public class MonthlyRepetition implements RepetitionStrategy {
    @Override
    public void repeat(TransactionManager transactionManager, Transaction transaction, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            transactionManager.addTransaction(new Transaction(transaction.getDescription() + " (Monthly)", transaction.getAmount(), calendar.getTime()));
        }
    }
}
